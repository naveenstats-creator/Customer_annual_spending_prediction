...
**Dataset:** 1,200 customers from an e-commerce platform, covering
demographics, membership/engagement behaviour, and `Annual_Spending`
as the target. `data/generate_data.py` builds `data/customer_spending.csv`
from the raw source data used for this project. To use your own data,
just point `DATA_PATH` in `src/analysis.py` at a CSV with the same
schema (see below).
...
│   └── customer_spending.csv   # cleaned dataset (1200 rows)
├── src/
│   └── analysis.py             # full pipeline (steps 1-10 above), script form
├── notebooks/
│   └── analysis.ipynb          # same pipeline, interactive/exploratory, pre-executed with outputs
├── results/
...
# 1. Build the dataset (skip if you already have data/customer_spending.csv)
python data/generate_data.py

# 2. Run the full analysis pipeline (script form -> results/ + saved figures)
python src/analysis.py

# 3. Or explore interactively (same pipeline, cell-by-cell, inline plots)
jupyter notebook notebooks/analysis.ipynb
```

Running `src/analysis.py` regenerates everything in `results/` and
prints a step-by-step log to the console. `notebooks/analysis.ipynb`
covers the same nine stages interactively and is already executed with
saved outputs, so it also renders directly on GitHub without needing to
be re-run.
